...
; this version plots three panels
...
xyouts,mean(xr),yr[1]-0.3,textoidl('Primary as AGN'),$
	align=0.5,/data,charsize=labelsize
xyouts,mean(xr),yr[1]-0.5,textoidl('(r_p < ')+strc(maxsep)+' kpc)',$
	align=0.5,/data,charsize=labelsize
...
mkplot,x,y,idx,xgri,xerr,phi_obs,phi_exp,ytit=ytit,xtickname=blanks,$
	xtickint=1,xminor=5,ytickint=1.0,yminor=5,/xs,/ys,xr=xr,yr=yr
xyouts,mean(xr),yr[1]-0.3,textoidl('Secondary as AGN'),$
	align=0.5,/data,charsize=labelsize
xyouts,mean(xr),yr[1]-0.5,textoidl('(r_p < ')+strc(maxsep)+' kpc)',$
	align=0.5,/data,charsize=labelsize
multiplot,/dox,/doy
