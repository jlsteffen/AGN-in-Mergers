...
for i=1,n_elements(sambit)-1 do begin
	...
	Ipagn = where((targpair.MANGA_TARGET1 and sambit[i]) ne 0 and $
		pcat.bclass[0] ge minbclass,Npagn) ; only count primaries
	Isagn = where((targpair.MANGA_TARGET1 and sambit[i]) ne 0 and $
		pcat.bclass[1] ge minbclass,Nsagn) ; only count primaries
	...
	xx = execute('wpagn = targpair[Ipagn].'+wnames[i])
	...
	wpagn_bs = bootstrap_mean(wpagn,nboot=nboot)
	...
	ncpagn = [total(wpagn),wpagn_bs * Npagn]/1e6
	...
	dlogncagn  = ( ncagn[3]- ncagn[1])/2/ ncagn[0]/alog(10)
        ...
        dlogncpagn = (ncpagn[3]-ncpagn[1])/2/ncpagn[0]/alog(10)
	...
	logexppagn = alog10(total(wpair*fagn_p[Ipair])/1e6)
	logexpsagn = alog10(total(wpair*fagn_s[Ipair])/1e6)
	logexpbagn = alog10(total(wpair*fagn_p[Ipair]*fagn_s[Ipair])/1e6)
	; placeholder for uncertainties
	dlogexpagn  = sqrt(dlognccntr^2+0.04^2)
	dlogexppagn = sqrt(dlogncpair^2+0.04^2)
	dlogexpsagn = sqrt(dlogncpair^2+0.04^2)
	dlogexpbagn = sqrt(dlogncpair^2+0.04^2)
	; excesses
	logexp = alog10(ncpagn[0])-logexppagn
	dlogexp = sqrt(dlogncpagn^2+dlogexppagn^2)
	...
	printf,5,' '
	printf,5,' '
	printf,5,f=fmt2,logexp,dlogexp
