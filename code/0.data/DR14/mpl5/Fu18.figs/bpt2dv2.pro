...
s = where((pcat.mngtarg1 and 2L^10+2L^11+2L^12) ne 0 and $ ; 149
	...
	pcat.NSA_ELPETRO_ABSMAG[5]+5*alog10(0.7) lt Mi_cut and $ ; 105
	min(pcat.bclass,dim=1) ge 2) ; 14
pcat = pcat[s]
...
pdir = '../9.bagn/'
forprint,'\plotone{'+pdir+outdir+plateifus[0:5]+'.eps}',$
	text=outdir+'part1.tex',/nocomment 
forprint,'\plotone{'+pdir+outdir+plateifus[6:*]+'.eps}',$
...
;for i=0,0 do begin
	if i eq 0 or i eq 6 then cbar = 1 else cbar = 0
	...
	putast,h_manga,[[sxpar(vbin.hdr,'CD1_1'),0],[0,sxpar(vbin.hdr,'CD2_2')]], $
		...
		['RA---TAN','DEC--TAN']	
	; match to SDSS image - pixel size small enough to avoid
	; antialiasing
	...
	manga_mkplot,map2,zr=[-0.5,2.5],colorbar=cbar,$ 
		...
		ztit='H'+cgSymbol('alpha')+' Surface Brightness',$
	...
	hastrom,map*mask,h_manga,map2,h2,hdr,interp=0
	manga_mkplot,map2,zr=[-450,450],colorbar=cbar,$ 
		;ztit='V (H'+cgSymbol('alpha')+') (km/s)',$
		ztit=textoidl('Gas Velocity (km/s)'),$
		xr=xr,yr=yr,xtit='',ytit='',$
		xtickint=10,xminor=5,ytickint=10,yminor=5,$
		position=!p.position,sauron=ctable,xtickname=blank,$
	...
	; Ha velocity dispersion map
	ctable = 1 
	map = manga_mkmap(binmap,pars.sigma[iha,0]) 
	hastrom,map*mask,h_manga,map2,h2,hdr,interp=0
	manga_mkplot,map2,zr=[0,300],colorbar=cbar,$ 
		;ztit=cgSymbol('sigma')+' (H'+cgSymbol('alpha')+') (km/s)',$
		ztit=textoidl('Gas Velocity Dispersion (km/s)'),$
		xr=xr,yr=yr,xtit='',ytit='',$
		xtickint=10,xminor=5,ytickint=10,yminor=5,$
		position=!p.position,sauron=ctable,xtickname=blank,$
		ztickint=100,zminor=5,$		
		c_map=c_map,c_levels=c_levels
	oplot,bundles[nfiber]*0.5*cos(a),bundles[nfiber]*0.5*sin(a),$
		color=cgcolor('dark gray'),lines=2
	multiplot,/dox
	...
	; NII/Ha map
	ctable = 1 
	map = manga_mkmap(binmap,n2ha) 
	hastrom,map*mask,h_manga,map2,h2,hdr,interp=0
	manga_mkplot,map2,zr=[-0.8,0.3],colorbar=cbar,$ 
		ztit='log([N II]/H'+cgsymbol('alpha')+')',$
		xr=xr,yr=yr,xtit='',ytit='',$
		xtickint=10,xminor=5,ytickint=10,yminor=5,$
		position=!p.position,sauron=ctable,xtickname=blank,$
		ztickint=0.4,zminor=4,$		
		c_map=c_map,c_levels=c_levels 		
	oplot,bundles[nfiber]*0.5*cos(a),bundles[nfiber]*0.5*sin(a),$
		color=cgcolor('dark gray'),lines=2
	multiplot,/dox
	...
	for code=1,4 do begin
		ind = where(bptcode[m1d] eq code,ct2)
	...
	plots,x[0],y[0],psym=cgSymCat(15),syms=2,color=cgcolor('white')
	plots,x[1],y[1],psym=cgSymCat(16),syms=2,color=cgcolor('white')
