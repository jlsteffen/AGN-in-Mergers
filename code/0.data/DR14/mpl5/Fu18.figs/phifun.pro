...
pro calc_phi,x,w,xgri,xbin,xi,flg_eagn,flg_sagn,flg_bagn,fagn_p,fagn_s,$
...
;    upper limit.
	xnp = n_elements(xgri)
	phi_bagn_exp = fltarr(xnp,3) ; binary AGNs
	phi_bagn_obs = fltarr(xnp,3)
	phi_eagn_exp = fltarr(xnp,3) ; either AGNs 
	phi_eagn_obs = fltarr(xnp,3) 
	phi_sagn_exp = fltarr(xnp,3) ; single AGNs 
	...
	for i=0,n_elements(xgri)-1 do begin
		...
		if npair eq 0 then continue
	
		; expected volume densities
		ind = ind_pair
		ct = npair
		; binary AGNs
		wbagn = (1-xi[i])*w[ind]*fagn_p[ind]*fagn_s[ind] +$
			xi[i]*w[ind]*fagn_p[ind] + $
			xi[i]*w[ind]*fagn_s[ind]
		phi_bagn_exp[i,*] = bootstrap_mean(wbagn,nboot=nboot)*ct
		; either AGN
		weagn = w[ind]*fagn_p[ind]+$
			w[ind]*fagn_s[ind]-$
			w[ind]*fagn_p[ind]*fagn_s[ind]
		phi_eagn_exp[i,*] = bootstrap_mean(weagn,nboot=nboot)*ct
		; single AGN
		wsagn = w[ind]*fagn_p[ind]*(1-fagn_s[ind])+$
			w[ind]*fagn_s[ind]*(1-fagn_p[ind])
		phi_sagn_exp[i,*] = bootstrap_mean(wsagn,nboot=nboot)*ct
		...
		; either AGN
		ind = where(flg_eagn and insidebin,neagn)
		phi_eagn_obs[i,*] = calc_phi_obs(w,ind,ind_pair,nboot)
		; single AGN
		ind = where(flg_sagn and insidebin,nsagn)
		phi_sagn_obs[i,*] = calc_phi_obs(w,ind,ind_pair,nboot)

		; print results
		print,xgri[i],neagn,nsagn,nbagn,npair		
	endfor
