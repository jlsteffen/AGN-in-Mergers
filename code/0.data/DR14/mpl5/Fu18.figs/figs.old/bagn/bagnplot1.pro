...
basename = exfilename(file_search(dir+'*.png'),/noext)
infile = '$MANGA_DIR/spectro/hfdap/mpl5/spfit/'+basename+'.fits'
...
;for i=0,1 do begin
	...
	binmap = vbin.binmap
	hdr = vbin.hdr
	xr = [-0.5,0.5]*(size(binmap))[1]*sxpar(hdr,'cd2_2')*3600
	yr = [-0.5,0.5]*(size(binmap))[2]*sxpar(hdr,'cd2_2')*3600
	pxsize = (sxpar(hdr,'cd2_2')*3600)^2

	; match SDSS_img w/ cube WCS for contour plot
	hastrom,total(sdss_img,1),h_sdss,sdss2,h2,hdr,interp=2
	c_map = sdss2
	c_levels = max(sdss2)*range(0.15,0.9,5)

	; H-alpha A/N mask
	ha_aon = manga_mkmap(binmap,pars.aon[iha])
	mask = ha_aon*0+!values.f_nan 		; 2D mask
	mask[where(ha_aon gt 3.0)] = 1.0
	m1d = where(pars.aon[iha] gt 3.0) 	; 1D mask

	; Ha velocity map
	ctable = 1	
	map = manga_mkmap(binmap,pars.vel[iha,0]) 
	position=!p.position
	manga_mkplot,map*mask,zr=[-400,400],colorbar=cbar,$
	...
	;position=!p.position
	;manga_mkplot,map*mask,zr=[0,250],colorbar=cbar,$
	;	ztit=textoidl('\sigma (H\alpha ) (km/s)'),$
	;	xr=xr,yr=yr,xtit='',ytit='',$
	;	xtickint=5,xminor=5,ytickint=10,yminor=5,$
	;	position=!p.position,sauron=ctable,xtickname=blank,$
	;	ztickint=50,zminor=5,$		
	;	c_map=c_map,c_levels=c_levels 		
	;multiplot,/dox
	...
	position=!p.position
	manga_mkplot,map*mask,zr=[-2.0,2.0],colorbar=cbar,$
	...
	position=!p.position
	manga_mkplot,map*mask,zr=[-1.0,1.0],colorbar=cbar,$
	...
	position=!p.position
	manga_mkplot,map*mask,zr=zr,colorbar=cbar,$
