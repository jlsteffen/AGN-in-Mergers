...
drpall = fits.getdata(datapath+'/DR14/drpall-v2_1_2.fits')
weight = fits.getdata(datapath+'/DR14/DR14_newcosm.fits')
pair = fits.getdata(datapath+'/DR14/mpl5/7.sample/pair1kpc.fits')

sepkpc = pair['sep_kpc']

z = pair['nsa_z']
mnsa = pair['nsa_elpetro_mass']/0.7**2

ew = []
for i in range(len(pair)):
    ix = (drpall['plateifu']==pair['plateifu'][i])
    iy = weight['nsa_nsaid']==drpall['nsa_nsaid'][ix]
    if len(weight['esweight'][iy]) == 0:
        ew.append(0)
    else:
        ew.append(weight['esweight'][iy][0])
ew = np.array(ew)

mstar = pair['mstar']
dm = mstar[:,0] - mstar[:,1]
vstar = pair['vstar']
dv = np.abs(vstar[:,0] - vstar[:,1])
...
par = (sepkpc<=40)&(dv <= 500)&(np.abs(dm)<=1.0)&(pair['sep_arcsec']<=pair['ifurin'])

bpt = pair['bpt']
bpt[pair['wha']<6] = 0

e = par&((bpt[:,0]>1)|(bpt[:,1]>1))
s = par&(((bpt[:,0]>1)&(bpt[:,1]<=1))|((bpt[:,1]>1)&(bpt[:,0]<=1)))
d = par&((bpt[:,0]>1)&(bpt[:,1]>1))
...
for i in range(len(rpbin)-1):
    # observed
    ix = (par==1)&(sepkpc<rpbin[i+1])&(sepkpc>rpbin[i])&(~np.isnan(ew))&(ew!=-99)&(~np.isnan(fagn))         
...
plt.savefig(filepath + '/3.vol_den_sep_Fu18.pdf', bbox_inches='tight')
plt.close(fig)